...
// Routing Logic & Simulated Authentication
function handleAuthRoute(event, action) {
    if(event) event.preventDefault();
    
    // Always permit signup or login routing
    if (action === 'signup' || action === 'login') {
        if(isLoggedIn) return showToast("You are already logged in!");
        document.getElementById('modal-title').innerText = action === 'signup' ? 'Create Account' : 'Login';
        document.getElementById('auth-modal').classList.add('active');
        return;
    }

    // For actions requiring authentication (finding matches, depositing, free play, ranked play, etc.)
    if (!isLoggedIn) {
        document.getElementById('modal-title').innerText = 'Authentication Required';
        document.getElementById('auth-modal').classList.add('active');
    } else {
        // Handle logic for logged in user -> route to designated area
        if (action === 'ranked') showToast("Routing to Ranked Match Lobby...");
        if (action === 'free') showToast("Routing to Free Play Lobby...");
        if (action === 'wallet') showToast("Opening Secure Wallet...");
